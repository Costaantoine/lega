...
TONY_MODEL = "qwen3:0.6b"
LEA_MODEL = "qwen3:0.6b"
...
# Tarif par agent (affiché dans le message upsell) — noms réels agent_registry
...
# Mapping legacy (noms prompt Tony) → noms réels agent_registry
TONY_AGENT_MAP: dict[str, str] = {
    "max_search": "scout", "sam_comms": "mel", "visa_vision": "iris",
    "documentation": "doc", "logistique": "logi", "comptable": "compta",
    "traducteur": "babel", "demandes_prix": "nego", "site_manager": "admin",
    "lea_extract": "lea",
    "agenda": "cal",
}
# Tous les noms valides dans agent_registry
_VALID_AGENTS: set[str] = set(TONY_AGENT_MAP.values()) | {
    "tony", "lea", "cal", "bell", "xray",
}
...
def detect_language(text: str, client_lang: str = None) -> str:
    ...
    nl_kw = {"graafmachine","kraan","lader","kopen","verkopen","zoek","prijs",
             "hallo","goedendag","bulldozer","heftruck","wiellader"}
    fr_kw = {"bonjour","pelleteuse","excavatrice","grue","tracteur","chargeuse",
             "niveleuse","compacteur","tombereau","nacelle","engin","engins",
             "machine","machines","travaux publics","tp","prix","recherche",
             "transport","livraison","devis","catalogue","acheter","vendre",
             "commercial","conseiller","parler","contact","demande","urgence",
             "renseignement","combien","coût","tarif","puis","pouvez","voulez"}
    scores = {
        ...
        "fr": sum(1 for w in fr_kw if w in t),
    }
...
async def tony_classify(message: str, client_lang: str = None, history: list = None) -> dict:
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(45.0, connect=5.0)) as client:
            ...
            if start >= 0 and end > start:
                ...
                _agenda_kw = {"agenda","planning","événement","tâche prévue","today","brief",
                              "calendrier","events","eventos","tarefas","aujourd'hui",
                              "mon agenda","mon planning","ma journée","agenda du jour",
                              "brief matinal","briefing","my schedule","meu dia",
                              "que faire","quoi faire","programme du jour",
                              "rendez-vous","réunion","reunião","meeting","compromisso"}
                _standardiste_kw = {"parler à un commercial","parler à un conseiller",
                                    "mise en relation","contact commercial","humain",
                                    "conseiller","commercial","contacter","transfert",
                                    "parler à quelqu'un","parler à une personne",
                                    "être mis en relation","parler au patron",
                                    "parler au gérant","parler à antoine",
                                    "falar com um comercial","falar com um consultor",
                                    "contactar comercial","transferir para humano",
                                    "talk to a salesperson","talk to a human",
                                    "speak to a representative","transfer to human",
                                    "can i speak to","i want to talk to"}
                ...
                elif any(w in m_low for w in _agenda_kw):
                    result.update({"intent":"agenda","agent":"agenda","direct_response":None,
                        "estimated_delay":"10s",
                        "ack_message": result.get("ack_message") or {"fr":"Je consulte votre agenda...","pt":"A consultar a sua agenda...","en":"Checking your agenda..."}.get(forced_lang,"Je consulte...")})
                elif any(w in m_low for w in _standardiste_kw):
                    result.update({"intent":"standardiste","agent":"bell","direct_response":None,
                        "estimated_delay":"8s",
                        "ack_message": result.get("ack_message") or {"fr":"Je vous mets en relation...","pt":"Vou transferir a chamada...","en":"Connecting you..."}.get(forced_lang,"Je vous mets en relation...")})
                ...
                if result.get("intent") == "general_chat" and any(w in message.lower() for w in machine_kw):
                    ...
                    result["ack_message"] = result.get("ack_message") or ack_by_lang.get(forced_lang, "Je recherche...")
                ...
                if result.get("intent") in ("general_chat",) and any(w in message.lower() for w in site_kw):
                    ...
                    result["ack_message"] = result.get("ack_message") or ack_by_lang.get(forced_lang, "Je modifie le site vitrine...")
                    result["estimated_delay"] = "5s"
                    result["direct_response"] = None
                return result
    except Exception as e:
        logger.warning(f"Tony classify error [{type(e).__name__}]: {e} | raw_len={len(raw) if 'raw' in dir() else 'N/A'}")
    # Fallback — agents spécialisés prioritaires, puis machine_search
    m_low = message.lower()
    # machine_search AVANT tout — si on parle de machines, scout d'abord
    machine_kw = {"escavadora","excavadora","pelleteuse","excavatrice","excavator","grue","crane",
                  "tracteur","tractor","chargeuse","loader","pelle","engin","engins",
                  "machine tp","machines tp","bulldozer","compacteur","tombereau","nacelle",
                  "chariot élévateur","chargeur","niveleuse","komatsu","cat ","volvo","doosan",
                  "liebherr","hitachi","jcb","case ","hyundai","semi-remorque","camion benne"}
    if any(w in m_low for w in machine_kw):
        ...
        return {
            ...
            "ack_message": ack_by_lang.get(forced_lang, "Je recherche..."), "estimated_delay": "2-4 min",
            "direct_response": None,
        }
    _logistique_kw = {"coût transport","combien transport","transport de","transport d",
                      "transport","livraison de","devis transport","fret","camion plateau","ro-ro",
                      "bateau ro","acheminer","expédier","convoi","logistique","shipping cost",
                      "custo transporte","transporte de","enviar máquina","calculer le transport",
                      "transport jusqu","transport depuis","porto","portugal"}
    _comptable_kw  = {"génère un devis","générer un devis","faire un devis","créer un devis",
    ...
    _agenda_kw = {"agenda","planning","événement","tâche prévue","today","brief",
                  ...
                  "que faire","quoi faire","programme du jour",
                  "rendez-vous","réunion","reunião","meeting","compromisso"}
    if any(w in m_low for w in _agenda_kw):
        return {"intent":"agenda","lang":forced_lang,"agent":"agenda",
                "ack_message":{"fr":"Je consulte votre agenda...","pt":"A consultar a sua agenda...","en":"Checking your agenda..."}.get(forced_lang,"Je consulte..."),
    ...
    _standardiste_kw = {"parler à un commercial","parler à un conseiller",
                        ...
                        "speak to a representative","transfer to human",
                        "can i speak to","i want to talk to"}
    if any(w in m_low for w in _standardiste_kw):
        return {"intent":"standardiste","lang":forced_lang,"agent":"bell",
                "ack_message":{"fr":"Je vous mets en relation...","pt":"Vou transferir a chamada...","en":"Connecting you..."}.get(forced_lang,"Je vous mets en relation..."),
                "estimated_delay":"8s","direct_response":None}
    _site_kw = {"ajoute produit","ajouter produit","ajoute une annonce","ajouter annonce",
                "crée une fiche","créer fiche","nouveau produit","ajouter au catalogue",
                "modifie le statut","modifier le statut","passe le produit","mettre en vente",
                "modifie le site","modifier le site","change le slogan","changer slogan",
    ...
    if direct:
        try:
            await ws.send_json({
                "type": "agent_response",
                "payload": direct,
                "metadata": {"intent": "general_chat", "lang": lang, "agent": "tony_interface", "done": True},
            })
        except Exception:
            pass
        return direct
...
async def tony_dispatch(
    ...
    try:
        ...
        if classification is None:
            # Fallback rapide sans appel Ollama — direct_response évite handle_general_chat
            lang = detect_language(user_msg, client_lang)
            classification = {
                "intent": "general_chat", "lang": lang, "agent": None,
                "direct_response": {
                    "fr": "Je coordonne 13 agents IA pour les machines TP. Que puis-je faire pour vous ?",
                    "pt": "Coordeno 13 agentes IA para máquinas TP. Em que posso ajudar?",
                    "en": "I coordinate 13 AI agents for construction machinery. How can I help?",
                    "es": "Coordino 13 agentes IA para maquinaria TP. ¿En qué puedo ayudarle?",
                }.get(lang, "Je coordonne 13 agents IA pour les machines TP. Que puis-je faire pour vous ?"),
            }
        intent = classification.get("intent", "general_chat")
        lang   = classification.get("lang", "fr")
        agent  = classification.get("agent")
        # Normaliser le nom d'agent via le mapping
        if agent and agent in TONY_AGENT_MAP:
            agent = TONY_AGENT_MAP[agent]
        elif agent and agent not in _VALID_AGENTS:
            logger.warning(f"Tony classify → agent inconnu '{agent}', fallback general_chat")
            agent = None
            intent = "general_chat"
...
async def run_lea_streaming(message: str, lang: str, canal: str, websocket: "WebSocket") -> None:
    """Agent Léa. Envoie un message d'attente immédiat (waiting_message, 0.1s),
    puis la réponse complète via gemma2 (stream=False, fiable).
    Web → gemma2:2b. Voice → gemma4:e2b.
    """
    import base64
    ...
    model = AGENT_MODEL if is_voice else LEA_MODEL

    _lang_display = {
        "pt": "European Portuguese (PT-PT)", "fr": "French", "en": "English",
        "es": "Spanish", "de": "German", "it": "Italian", "nl": "Dutch",
        "ru": "Russian", "ar": "Arabic", "zh": "Chinese (Simplified)",
    }
    lang_display = _lang_display.get(lang, "European Portuguese (PT-PT)")

    # ── Étape 1: message d'attente immédiat (zéro LLM, 0.1s) ──
    if not is_voice:
        _WAITING = {
    ...
    # ── Fetch catalogue (30 max) ──
    _price_on_request = {
        "pt": "price on request", "fr": "price on request", "en": "price on request",
        "es": "price on request", "de": "price on request", "it": "price on request",
        "nl": "price on request", "ru": "price on request", "ar": "price on request",
        "zh": "price on request",
    }
    ...
    catalogue_ctx = ""
    try:
        conn = await asyncio.wait_for(db_connect(), timeout=8.0)
        try:
            ...
            if rows:
                ...
                catalogue_ctx = f"\nCATALOGUE ({len(lines)} machines — search this list):\n" + "\n".join(lines)
        finally:
            await conn.close()
    except (asyncio.TimeoutError, Exception):
        logger.warning("lea catalogue: timeout, réponse sans catalogue")

    # ── Étape 2: appel Ollama unique (gemma2 ou gemma4 selon canal) ──
    rules = (
    ...
    prompt = (
        ...
        f"RULES: {rules}\n"
        f"USER: {message}\n"
        f"LÉIA (respond in {lang_display} only, reference exact machines from CATALOGUE):"
    )
    ...
    try:
        ...
        logger.error(f"lea error ({canal}): {type(e).__name__}: {e}")
...
async def run_agenda(payload: dict, lang: str) -> str:
    """Agent Agenda: répond aux questions sur le planning/agenda via build_morning_brief."""
    try:
        user_prefs = payload.get("preferences", {})
        if isinstance(user_prefs, str):
            try:
                user_prefs = json.loads(user_prefs)
            except Exception:
                user_prefs = {}
        return await build_morning_brief(lang=lang, user_prefs=user_prefs)
    except Exception as e:
        logger.error(f"run_agenda error: {e}")
        fallback = {
            "fr": "📅 Voici votre agenda du jour. Aucun événement planifié pour le moment.",
            "pt": "📅 Aqui está a sua agenda de hoje. Sem eventos planeados de momento.",
            "en": "📅 Here's your agenda for today. No events scheduled at the moment.",
        }
        return fallback.get(lang, fallback["fr"])
...
AGENT_EXECUTORS = {
    ...
    "admin": run_site_manager,
    "lea": lambda payload, lang: run_standardiste(payload.get("message", ""), lang),
    "bell": lambda payload, lang: run_standardiste(payload.get("message", ""), lang),
    "logi": run_logistique,
    "compta": run_comptable,
...
async def create_task(session_id: str, agent_name: str, payload: dict, lang: str, estimated_latency: int) -> str:
    ...
    # Fallback si agent_name est None (classify failed)
    safe_agent = agent_name or "general_chat"
    ...
    try:
        await conn.execute(
            ...
            task_id, session_id, safe_agent, json.dumps(payload_stored), lang, estimated_latency,
        )
    finally:
        await conn.close()
    return task_id
